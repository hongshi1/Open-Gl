    ...
    while (!fileStream.eof())
    {
        ...
        if (line.compare(0, 1, "f") == 0) // 原书有误
        {
            ...
            while (getline(ss, oneCorner, ' ')){
                ...
                std::string v, t, n;
                getline(cornerSS, v, '/');
                _triangleVerts.push_back((std::stoi(v) - 1)*3);
                if (getline(cornerSS, t, '/'))
                {
                    if (!t.empty())
                    {
                        _textureCoords.push_back((std::stoi(t) - 1)*2);
                    }
                    else
                    {
                        _textureCoords.push_back(-1); // 纹理坐标缺失的标记
                    }
                }

                if (getline(cornerSS, n, '/'))
                {
                    _normals.push_back((std::stoi(n) - 1)*3);
                }
            }
